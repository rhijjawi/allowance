...
export class Debt {
    ...
    constructor( name : string, balance: number, interestRate : number, minimumPayment?: number) {
...
export function createRepaymentPlan(debts : Debt[], strategy = 'avalanche', totalMonthlyPayment : number) {
    switch (strategy) {
        case 'avalanche':
...
export function snowballStrategy(debts : Debt[], totalMonthlyPayment : number) {
  ...
  export function allocatePayments(debts : Debt[], totalMonthlyPayment : number) {
    ...
    return paymentPlan.map(({ debt, payment } : {debt : Debt, payment : number}) => ({ name: debt.name, payment })); // Return simplified plan for display
  }
