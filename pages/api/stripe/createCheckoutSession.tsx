...
async function POST(req: NextApiRequest, res: NextApiResponse){
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        line_items : item.map((prod : [string, boolean])=>({price: prod[0], quantity: 1})),
        customer: stripeId,
        subscription_data: {trial_period_days: 30},
        mode: item[0][1] ? 'subscription' : 'payment',
        allow_promotion_codes : true,
        success_url: afterSuccess ? `https://logmoney.app/success?session_id={CHECKOUT_SESSION_ID}&aftersucess=${afterSuccess}` : `https://logmoney.app/success?session_id={CHECKOUT_SESSION_ID}`,
