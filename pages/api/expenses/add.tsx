...
export default async function handler(
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        try {
            ...
            const f = await readFile(files.csvFile![0].filepath, {
                encoding: 'utf-8',
            })
            ...
            const { data, error } = await supabase
                ...
                .eq('user_id', userId)
                .select(
                    'amount,currency,category,label,transaction_date,recurring,files,refunded,is_displayed'
                )
            if (!error) {
                return res
                    .status(200)
                    .json({ results: data, count: result.length })
            } else {
...
function convertCsvToJson(
    ...
    let results = csv.splice(1).map((line: any) => {
        ...
        const jsonObject: ExpenseSchema = {
            ...
            transaction_date: bankData.transaction_date as Date,
            user_id: userId,
