...
export default function Manage() {
    ...
    useEffect(()=>{
        async function _(){
            let res;
            try{
                res = await axios.post(process.env.NODE_ENV == "development" ? "http://expenses.ramzihijjawi.me:3000/api/reports?page="+page : "https://logmoney.app/api/reports?page="+page)
            }
            catch (e) {
                setReports([])
                return null;
    ...
    return (
        <div className="h-fit w-full overflow-hidden border-t-2 bg-white dark:bg-dark-tremor-background-muted/75">
            ...
            <Card className={`mx-auto  my-auto mt-5 w-full max-w-[80%] divide-gray-200 rounded-lg border-2 shadow-2xl `}>
                ...
                {reports ? <div className={`h-fit my-5 grid grid-cols-1 auto-rows-fr`}>
                {reports.length > 0 ? <>
                    {reports.map((report : {childFor : string, uuid : string, date_range? : [number, number]}, index : number)=>{
                    return (
                        <>
                            <div key={index} className="px-1 h-fit py-2 mb-3 relative bg-slate-100/30  dark:bg-gray-700 border-black/70 border-2 rounded-md">
                ...
                {new Array(5-reports.length).fill(null).map((item, index)=>(
                    <>
                        <div className="px-1 py-2 mb-3 relative bg-slate-100/30 max-h-[200px]  dark:bg-gray-700 border-black/70 border border-dashed rounded-md" key={index+reports.length}>
                ...
                <div className="mx-auto w-36 relative h-6">
                    {(pages > 0 && page > 0) && <ArrowLeftIcon onClick={()=>{setPage((pg)=>pg-1)}} className="left-0 h-6 inline cursor-pointer"/>}
                    ...
                    {(pages > 0 && page+1 < pages) && <ArrowRightIcon onClick={()=>{setPage((pg)=>pg+1)}} className="h-6 absolute right-0  inline cursor-pointer"/>}
                </div>
            </Card>
            </div>
        </div>
