...
import { Button, Select, TextInput, SelectItem, Subtitle, Title, Card} from "@tremor/react";
                ...
                <div className="flex flex-col sm:flex-row">
                  <div className="relative flex-grow">
                    ...
                    <div className="mt-2 max-w-xl text-sm text-gray-500 dark:text-gray-400">
                      <p>
                        Configure reports to be sent to your parents or
                        guardians
                      </p>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-2">
                ...
                <div className="relative col-span-2 col-start-1 mx-auto mb-5 w-[80%]  rounded-md border-2 border-indigo-500 bg-indigo-200/50 px-4 py-4 shadow-sm dark:border-cyan-500">
                  ...
                  {supervisorData && user?.publicMetadata.role == "student" &&
                    ...
                    <div className="">
                    {supervisorData && user?.publicMetadata.role == "parent" &&
                      supervisorData.supervisorProfiles.filter((e)=>e !== null).map((user: User, index: number) => (
                        <UserCard key={index} user={user} supervisorDelete={async()=>{
                            const res = axios.delete("/api/user/assignParent",{data: {childId: user.id}});
                        }}/>
                    ))}
                    </div>
                  <UserAddCard isReadOnly={user?.publicMetadata.role == 'parent'}/>
                </div>
                  ...
                  settings.map((setting) => {
                    return (
                      <div className="relative col-span-2 col-start-1 mx-auto mb-5 w-[45%]  rounded-md border-2 border-indigo-500 bg-indigo-200/50 px-4 py-4 shadow-md dark:border-cyan-500">
                        ...
                        <Select
                          onValueChange={(val) => setting.onChange(val)}
                          value={setting.value}
                          enableClear={false}
                ...
                {user!.publicMetadata.role == "parent" && (
                  <div className="col-span-2 mx-auto w-56">
                    <Button
                      ...
                      onClick={async (e) => {
                        setIsLoading(true);
                        const metaupdate = await fetch("/api/user/metadata", {
                          method: "PUT",
