...
export default function Manage(){
    ...
    useEffect(() => {
        if (currency == null || currency === data?.metadata?.reports?.currency){return}
        fetch('/api/user/metadata', {
            ...
            addAlert("success", "Successfully changed report currency!", 5000)
        }).catch((err) => {
                ...
                <div className="w-full h-full bg-white rounded-md">
                    ...
                    <div>
                        ...
                        <div className="w-80 h-full flex flex-col justify-start items-center py-3 mx-auto">
                            <Select enableClear={false} className="" disabled={!isLoaded} value={isLoading ? "Loading..." : currency ? currency : data ? data['currency'] : "Select a currency"} onValueChange={(e : string) => {setCurrency(e)}}>
                                {Object.keys(currencies).map((key : string) => {
                                    return <SelectItem key={key} value={key}>({key}) {(currencies as any)[key]}</SelectItem>
                                })}
                            </Select>
