...
export default function Calculator() {
    ...
    }>({ monthlyBalances: null, totalInterestPaid: 0 })
    ...
    useEffect(() => {
        const { monthlyBalances, totalInterestPaid } = calc.simulateRepayments(
            avData,
            3000,
            'avalanche'
        )
                    ...
                    {snowData.map((debt: calc.Debt, index: number) => (
                        <>
                            <Card className="max-w-sm dark:ring-amber-300">
                                <p className="text-3xl mb-2 leading-6">
                                    {debt.name}
                                </p>
                ...
                <Card className="max-w-sm h-fit mb-12">
                    ...
                    <div className="grid grid-rows-2 grid-cols-1 gap-y-5">
                        <NumberInput
                            ...
                            min={0}
                            error={int == null || int > 9999999 || int < 0}
                            max={999999999999}
                            value={bal ? bal : undefined}
                            onValueChange={(e) => {
                                setBal(e)
                            }}
                        />
                        <NumberInput
                            placeholder="Debt Interest APY (%)"
                            min={0}
                            error={int == null || int > 99 || int < 0}
                            max={99}
                            step={'1'}
                            value={int ? int : undefined}
                            onValueChange={(e) => {
                                setInt(e)
                            }}
                        />
                    ...
                    <div className="w-full mt-5">
                        <button
                            ...
                            className="inline-flex disabled:cursor-not-allowed items-end justify-center rounded-md border border-transparent bg-green-100 px-4 py-2 text-sm font-medium text-green-900 dark:text-white dark:bg-black/20 ring-1 ring-green-400 hover:bg-green-200 dark:hover:bg-black/0 focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2"
                        >
                            Add
                        </button>
                    </div>
                ...
                <div className="grid grid-cols-2 grid-rows-1 gap-x-12">
                    <AreaChart
                        ...
                        categories={Object.keys(
                            avalanche.monthlyBalances[0]
                        ).filter((val) => val != 'month')}
                        stack={false}
                        valueFormatter={(val: number) =>
                            `Balance: ${currFormatter(val)}`
                        }
                    />
                    <AreaChart
                        showAnimation
