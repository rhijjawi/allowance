...
export async function getServerSideProps(
        ...
        return {
            props: {
                error: {
                    ...
                    message:
                        "We're sorry, but we weren't able to find the report you requested.",
                },
            },
        }
    }
        ...
        return {
            props: {
                error: {
                    ...
                    message:
                        'If you clicked a link to get here, send us a message using the button below',
                },
            },
        }
    }
    ...
    if (_expenses && !expensesError) {
        ...
        if (expenses.length > 0) {
            ...
            const currencies: string[] = filteredExpenses.flatMap(
                (element: ExpenseType) => {
            ...
            for (let curr of currencies) {
                !_currency[curr]
                    ? (_currency[curr] = await standardizeCurrencyGeneral(
                          1,
                          curr,
                          (_user.publicMetadata.reports as { currency: string })
                              .currency
                      ))
                    : null
            }
...
export default function Report(props: {
    ...
    const customTooltip = (_: any) => {
        ...
        return (
            <Card className="w-56 rounded-tremor-default border relative border-tremor-border bg-tremor-background p-2 text-tremor-default shadow-tremor-dropdown">
                    ...
                    return (
                        <div key={idx} className="flex flex-1 space-x-2.5">
                            ...
                            <div className="space-y-1">
                                <p className="text-tremor-content dark:text-white font-bold">
                                    {category.dataKey}
                                </p>
                ...
                <p className="text-tremor-content dark:text-slate-500 text-xs text-right">
                    {payload[0].payload.prettyDate}
                </p>
            </Card>
        )
    }
    ...
    useEffect(() => {
        ...
        sortedExpenses.forEach((expense) => {
            let category =
                categories.find((element: any) => {
                    return element.id === expense.category[0]
                }) ?? undefined
            if (category) {
                expenseList[category.id]
                    ? null
                    : (expenseList[category.id] = { ...category, sum: 0 })
                expenseList[category.id].sum
                    ? null
                    : (expenseList[category.id].sum = 0)
                expenseList[category.id].sum +=
                    expense.amount * props._currency[expense.currency]
                ...
                _dayByDay[Number(newDate)]
                    ? (_dayByDay[Number(newDate)].sum +=
                          expense.amount * props._currency[expense.currency])
                    : (_dayByDay[Number(newDate)] = {
                          sum:
                              expense.amount *
                              props._currency[expense.currency],
                      })
            }
        })
        let previous = 0
        const dateArray = Object.keys(_dayByDay).map((key, index: number) => {
            const a = {
                ...
                Expenditure:
                    index > 0
                        ? _dayByDay[key].sum +
                          Object.values(_dayByDay)[index - 1].sum
                        : _dayByDay[key].sum,
                prev: previous,
        ...
        return (
            <div className="w-full h-[96vh] relative overflow-hidden border-t-2 bg-white dark:bg-dark-tremor-background-muted/75">
                <div className="w-fit h-fit absolute right-0 left-0 top-0 bottom-0 my-auto mx-auto">
                    <svg
                        aria-hidden="true"
                        className="w-12 h-12 mb-12  text-gray-200 animate-spin mx-auto dark:text-gray-600 fill-purple-600"
                        viewBox="0 0 100 101"
                        fill="none"
                    ...
                    <p>
                        This can sometimes take a while. If you run into any
                        issues, report a bug using the button below.
                    </p>
                </div>
                <span className="sr-only">Loading...</span>
            </div>
        ...
        return (
            <>
                {selectedExpenses.length > 0 && (
                    <Card className="fixed min-h-[4rem] max-h-fit max-w-fit z-50 border-neutral-900 border rounded-md bottom-5 left-5">
                        <p>
                            Average:{' '}
                            <strong>
                                {currFormatter(
                                    props.expenses
                                        .filter((exp) => {
                                            return selectedExpenses.includes(
                                                exp.id!
                                            )
                                        })
                                        .map((exp) => {
                ...
                <div className="w-full overflow-hidden border-t-2 bg-white dark:bg-dark-tremor-background-muted/75">
                    <div className="mx-auto py-5 min-h-screen max-w-[88rem] px-6 lg:px-8">
                        <Card>
                            {props.parent.id == user.userId && (
                                <div className="h-12 absolute right-0 pr-6">
                                    <Button
                                        iconPosition="left"
                                        onClick={() => {
                                            navigator.clipboard
                                                ? navigator.clipboard.writeText(
                                                      `https://logmoney.app/report/${props.shareLink}`
                                                  )
                                                : alert(
                                                      `https://logmoney.app/report/${props.shareLink}`
                                                  )
                                        ...
                                        icon={ShareIcon}
                                        className=" float-right inline-flex justify-center rounded-md border-none bg-indigo-100 px-4 py-2 text-sm font-medium text-indigo-900 hover:bg-indigo-200 focus:outline-none  focus-visible:ring-red-500 focus-visible:ring-offset-2"
                                    >
                                        Share
                                    </Button>
                                </div>
                            ...
                            <div className="grid grid-cols-3 gap-y-5 w-full">
                                {Object.values(categoryData!).map((cat) => {
                                    return (
                                        ...
                                        sum[cat.id] && (
                                            <Card
                                                className={`bg-${getColor(cat.id)}-300 dark:border-2 dark:border-${getColor(cat.id)}-300 mx-auto w-[85%]`}
                                            >
