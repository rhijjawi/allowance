  ...
  useEffect(() => {
    ...
    if (data.length > 0) {
      if (active) {
        data = data.sort(
          (a: any, b: any) =>
            Date.parse(a.transaction_date) - Date.parse(b.transaction_date),
        );
        setChartData(data);
      }
      return () => {
      ...
      >
        ...
        >
          ...
          <p className="text-left text-2xl font-semibold">
            {currFormatter(misc.emergency[0], misc.emergency[2])} in cash
          </p>
          <p className="text-left text-2xl font-semibold">
            {currFormatter(misc.emergency[1], misc.emergency[2])} in bank
            account
          </p>
          <EmergencyFundModal
            misc={misc}
