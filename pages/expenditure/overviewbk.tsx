...
export default function Expenditure() {
  ...
  useEffect(() => {
    ...
    sorted.map((i: ExpenseType) => {
      !months.includes(
        new Date(Date.parse(i.transaction_date as string)).getUTCMonth(),
      )
        ? months.push(
            new Date(Date.parse(i.transaction_date as string)).getUTCMonth(),
          )
    ...
    months.forEach(async (i) => {
      let onlyMonths = sorted.filter((e: ExpenseType) => {
        return (
          new Date(Date.parse(e.transaction_date as string)).getUTCMonth() ==
            i && e.recurring == false
        );
      });
      let onlyRecurring = sorted.filter((e: ExpenseType) => {
        return e.recurring;
      });
      onlyMonths.forEach(async (i) => {
        ...
        expenseList[
          `${NumToMonth(date.getUTCMonth())} ${date.getUTCFullYear()}`
        ]
          ? {}
          : (expenseList[
              `${NumToMonth(date.getUTCMonth())} ${date.getUTCFullYear()}`
      ...
      onlyRecurring.forEach(async (j) => {
        ...
        let category = categoryData.find((element: any) => {
          return element.id === j.category[0];
        })!.category;
        console.log(category);
        if (
          new Date(Date.parse(j.transaction_date as string)).getUTCMonth() <= i
        ) {
          expenseList[`${NumToMonth(i)} ${date.getUTCFullYear()}`]
            ? {}
            : (expenseList[`${NumToMonth(i)} ${date.getUTCFullYear()}`] = {});
          expenseList[`${NumToMonth(i)} ${date.getUTCFullYear()}`][category]
            ? (expenseList[`${NumToMonth(i)} ${date.getUTCFullYear()}`][
                category
              ] += await standardizeCurrency(
                j,
                user!.publicMetadata?.currency as string,
              ))
            : (expenseList[`${NumToMonth(i)} ${date.getUTCFullYear()}`][
    ...
    if (data.length > 0) {
      if (active) {
        data = data.sort(
          (a: any, b: any) =>
            MonthToNum(a.month.split(" ")[0]) -
            MonthToNum(b.month.split(" ")[0]),
        );
        setChartData(data);
      }
      return () => {
  ...
  return (
    <main className="bg-dark-tremor-background-muted/75 -z-[100] flex min-h-screen flex-col items-center justify-between px-24 pt-12">
      ...
      >
        ...
        >
          ...
          {misc.budget[0] !== 0 ? (
            <Subtitle className={`dark: text-center`}>
              You have spent <b>{percentage[0].toFixed(2)}</b>% of your budget
            </Subtitle>
          ) : null}
        </CustomCard>
        <CustomCard
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 top-0 flex h-full w-full justify-center align-middle">
            {misc.savings[1] == 0 ? (
              <div className="relative my-auto">
                <Text className="mx-auto w-fit px-4">
                  To manage your savings, you'll need to set a goal first.
                </Text>
              </div>
            ) : null}
          </div>
              ...
              {misc.savings[1] == 0 ? null : (
                <>
                  <p className="text-center text-3xl font-semibold">
                    {currFormatter(misc.savings[0], misc.savings[2] as string)}{" "}
                    saved
                  </p>
                  <ProgressBar
                    tooltip={`${(misc.savings[0] / misc.savings[1]) * 100}%`}
                    className=" h-full rounded-md"
                    value={(misc.savings[0] / misc.savings[1]) * 100}
                  />
                </>
              )}
            </div>
            {misc.savings[1] !== 0 && (
              <p className="w-full text-center text-3xl font-semibold">
                <span className="text-green-400">
                  {currFormatter(misc.savings[1], misc.savings[2] as string)}
                </span>{" "}
                goal
              </p>
            )}
        ...
        <CustomCard className="relative" variants={cardVariants}>
            ...
            <div className="absolute right-0 top-0 px-12 py-5">
              ...
              >
                <label className="cursor-pointer  text-sm font-semibold ">
                  ...
                  <span className="font-bold ">Rent</span> category
                </label>
              </div>
            </div>
