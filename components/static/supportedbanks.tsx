...
export const functions : {[index : string] : (arr : string[]) => InsertExp|null} = {
...
class Transaction {
    ...
    constructor(line : string[], label: number, amount: number, currency: number, category : [number, number]|null,  refunded: boolean, transaction_date: number, recurring: boolean, dateFormat : string, files: string[], is_displayed?: boolean)  {
        ...
        this.transaction_date = fnsparse(line[transaction_date], dateFormat, new Date())
...
function Wise(arr : string[]){
    ...
    const transaction = new Transaction(arr, 4, 2, 3, null, false, 1, false, "dd-MM-yyyy", [])
...
function CommerzBank(arr : string[]){
    if (arr.length > 0){
        if (arr[2] == "debit"){
            const transaction = new Transaction(arr, 3, 4, 5, null, false, 0, false, "dd.MM.yyyy", [])
