...
export default function buttonWhenHoverShowOrigCurr(props : {expense : ExpenseSchema, size: "sm" | "md" | "xs" | "lg" | "xl"}){
    ...
    let CurrString = [`${currFormatter(props.expense.amount, props.expense.currency)}`, `${currFormatter(props.expense.standardizedCurrency!, user!.publicMetadata!.currency as string)}`][Number(isMouseOut)]
...
export function HoverCurrGeneral(props : {expense : ExpenseSchema, size:  "sm" | "md" | "xs" | "lg" | "xl",  currency: [string, number]}){
    const [isMouseOut, setIsMouseOut] = useState<boolean>(true)
    const {user} = useUser()
    
    let CurrString = [`${currFormatter(props.expense.amount, props.expense.currency)}`, `${currFormatter(props.expense.amount * props.currency[1]!, props.currency[0] as string)}`][Number(isMouseOut)]
    return <Button variant="secondary" color="gray" size={props.size} className={`w-32 cursor-default hover:bg-transparent border-2 dark:border-red-600`} onMouseEnter={()=>{setIsMouseOut(false)}} onMouseLeave={()=>{setIsMouseOut(true)}}>
        <div className="w-full dark:text-white text-tremor-content">{CurrString}</div></Button>
}