...
export default function MIMO(props: {
    income: IncomeSchema[]
    currency: string
}) {
    ...
    useEffect(() => {
        async function handle() {
            ...
            expenseData.map((expense: ExpenseType) => {
                if (
                    new Date(expense.transaction_date).getUTCMonth() ==
                        new Date().getUTCMonth() ||
                    expense.recurring == true
                ) {
                    setmoneyOut((prev) => {
                        return prev + expense.standardizedCurrency!
                    })
                }
            })
            props.income.map((income: IncomeSchema) => {
                if (
                    new Date(income.transaction_date).getUTCMonth() ==
                        new Date().getUTCMonth() ||
                    income.recurring == true
                ) {
                    setMoneyIn((prev) => {
                        return prev + income.standardizedCurrency!
                    })
                }
            })
        }
