...
export default function DC(props: any) {
    ...
    useEffect(() => {
            ...
            for (let exp of expenseData) {
                if (
                    new Date().getUTCMonth() ==
                        new Date(exp.transaction_date).getUTCMonth() ||
                    exp.recurring == true
                ) {
                    let catString = categoryData.find((element) => {
                        return element.id === exp.category[0]
                    })!.category
                    if (!catSums[catString]) {
                        catSums[catString] = 0
                    }
                    catSums[catString] =
                        catSums[catString] + exp.standardizedCurrency!
                }
            }
            return [catSums]
        }
        if (categoryData && user && expenseData) {
            let sumsArr: any = []
            handle().then((d) => {
                Object.values(d[0]).map((sum) => {
                    setTotalExpenditure((prev) => {
                        return prev + sum
                    })
                })
    ...
    return (
        <>
            ...
            <DonutChart
                ...
                className="h-64 py-5 mx-2 px-2 rounded-md "
                colors={donut.map((e: any) => {
                    return getColor(getIDByCategoryName(e.label, categoryData))
                })}
                category="value"
                variant="donut"
            ...
            />
            {slice ? (
                <Card className="py-5 px-2 min-h-12 mb-2 rounded-md dark:text-slate-300">
                    <Text className="leading-6 dark:text-slate-300">
                        {Badge} accounts for{' '}
                        {((slice?.value / totalExpenditure) * 100).toFixed(1)}%
                        of your total expenditure this month
                    </Text>
                </Card>
            ) : (
                <></>
