...
export function TransactionHandlerProvider({children} : {children: React.ReactNode}){
    ...
    const [handlerMode, setHandlerMode] = useState<[null|'delete'|'edit'|'modifyCategory'|'add', number[]]>([null, []])
    ...
    const functions : { [key: string]: () => Promise<any> } = {
        'delete' : async() : Promise<undefined> => {
            setIsLoading(true)
            await (await getSupabase(await getToken({template: "supabase"}))).from('expenses').delete().in('id', handlerMode[1])
            setExpenseData(expenseData.filter((e : ExpenseType) => handlerMode[1].indexOf(e.id!) == -1))
            if (handlerMode[1].length > 1) addAlert("success", `Successfully deleted ${handlerMode[1].length} transactions`)
        ...
        'modifyCategory' : async() : Promise<undefined|{data : null, error: any}> => {
            if (resolvedExpense.length == 0){return};
            const {data, error} = await (await getSupabase(await getToken({template: "supabase"}))).from('expenses').update({category: resolvedExpense[0].category}).in('id', handlerMode[1])
            if (!error){
                ...
                setExpenseData(expenseData.map((e : ExpenseType) => {
                    if (handlerMode[1].indexOf(e.id!) != -1){
                        return {...e, category : resolvedExpense[0].category}
                    }
                    return e
                }));
    ...
    useEffect(() => {
        setResolvedExpense(expenseData.filter((e : ExpenseType) => (handlerMode[1].indexOf(e.id!) !== -1)))
    }, [handlerMode])
    return (
        <TransactionHandlerCTX.Provider value={{handlerMode, setHandlerMode}}>
            ...
            <>    
          <Transition appear show={handlerMode[0] == "delete"} as={Fragment}>
                ...
                <div className="flex min-h-full items-center justify-center p-4 text-center">
                  ...
                  >
                    <Dialog.Panel className="w-full max-w-md transform  rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                      ...
                      >
                        Delete { resolvedExpense.length > 1 ? "these transactions" : "this transaction"}
                      </Dialog.Title>
                      <div className="mt-2"> 
                       <>
                        <p className="text-sm mb-2 text-gray-500">
                        Confirm that you'd like to delete { resolvedExpense.length > 1 ? `${resolvedExpense.length} transactions` : "this transaction"} using the buttons below.
                        </p>
                    ...
                    <div className="float-right">
                      <button
                        ...
                        onClick={async()=>{await functions["delete"]()}}
                      >
                        Confirm
                      </button>
                    </div>
