...
const ExpenseCTX = createContext<ExpenseProvider>(
    [] as unknown as ExpenseProvider
)

export function ExpenseCTXProvider({
    children,
}: {
    ...
    useEffect(() => {
        ...
        async function fetchExpenses() {
            ...
            const incomeCategories = await supabase
            ...
            if (income.data) {
                for (const _ of income.data) {
                    let standardizedCurrency = await standardizeCurrency(
                        _,
...
export function useExpenses() {
    ...
    if (ctx === undefined)
        throw new Error(
            'useExpenseCTX must be used within a ExpenseCTXProvider'
        )
    return ctx
}
