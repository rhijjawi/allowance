...
export function DragAndDrop(props: {
    ...
    useEffect(() => {
        ...
        async function getURLs() {
            const tempURLs = (
                await generateTemporaryUrl(props.exp.files, props.exp.id!)
    ...
    async function handleSubmitFile(e: any) {
        ...
        if (filesArr.length === 0) {
            ...
            for (let i = 0; i < filesArr.length; i++) {
                ...
                if (url) {
                    const tempURLs = (
                        await generateTemporaryUrl([key], props.exp.id!)
                    ).data as [string, string][]
                    tempURLs.forEach((url) => {
                        setFileURLs((prev) => ({ ...prev, [url[0]]: url[1] }))
                    })
                ...
                setFilesArr((prev) =>
                    prev.filter((file) => file != filesArr[i])
                )
                setExpenseData((prev: ExpenseType[]) =>
                    expenseData.map((exp) => {
                        let _data = Object.assign({}, exp) as ExpenseType
                        if (exp.id == props.exp.id) {
                            alert(
                                Object.keys(fileURLs).flatMap((file) =>
                                    file.split('/').slice(-1)
                                )
                            )
                            return {
                                ...exp,
                                files: Object.keys(fileURLs).flatMap((file) =>
                                    file.split('/').slice(-1)
                                ),
                            }
                        }
    ...
    function handleDrop(e: any) {
        ...
        if (e.dataTransfer.files && e.dataTransfer.files[0]) {
            for (let i = 0; i < e.dataTransfer.files['length']; i++) {
                console.log(
                    (e.dataTransfer.files[i] as File).size / 1000000 <= 5
                )
                setFilesArr((prevState: any) => [
                    ...prevState,
        ...
        <>
            <div className="grid grid-cols-2 text-lg">
                <div className="w-[90%]">
                    ...
                    <div className="text-black bg-gray-800/20 py-2 px-2 break-words border border-black rounded-md">
                        {props.exp.label}
                    </div>
                </div>
                <div className="w-[90%]">
                    ...
                    <div className="text-black bg-gray-800/20 py-2 px-2 break-words border border-black rounded-md">
                        ...
                        }).format(props.exp.amount)}
                    </div>
                </div>
                <div className="col-span-2">
                    <Text>
                        This transaction took place on{' '}
                        <Bold>
                            {new Date(
                                props.exp.transaction_date
                            ).toLocaleDateString()}
                        </Bold>
                        .
                    </Text>
                            ...
                            (key: string, idx: number) => {
                                return (
                                    <div className="w-fit max-w-full h-fit mx-auto">
                                        ...
                                        >
                                            <FolderOpenIcon className="h-20 aspect-square mx-auto text-indigo-400" />
                                            <p className="text-sm w-fit mx-auto text-center break-normal break-words whitespace-pre-wrap text-ellipsis max-w-[50%]">
                                                {key.split('/').splice(-1)}
                                            </p>
                                        </Link>
                                    </div>
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center p-3">
                            {filesArr.map((file: any, index: any) => (
                                ...
                                >
                                    <Text className="text-ellipsis whitespace-nowrap w-48 truncate">
                                        {file.name}
                                    </Text>
