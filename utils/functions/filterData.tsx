...
export function MonthExpenses(expenses: any, month: Date) {
    return expenses.filter((expense: any) => {
        ...
        if (
            ...
            date.getFullYear() == month.getUTCFullYear()
        ) {
            return true
        }
...
export function From1sttoNowEXP(
    expenses: ExpenseType[] | IncomeType[],
    category: CategorySchema
) {
    const lastMonthToDate = expenses.filter(
        (expense: ExpenseType | IncomeType) => {
            let today = new Date()
            ...
            return (
                firstDay <= new Date(expense.transaction_date) &&
                new Date(expense.transaction_date) <= lastDayAllowed &&
                expense.category[0] == category.id
            )
        }
    )
    const MonthToDate = expenses.filter((expense: ExpenseType | IncomeType) => {
        ...
        let lastDayAllowedLastMonth = new Date(
        ...
        return (
            firstDayLastMonth <= transaction_date &&
            transaction_date <= lastDayAllowedLastMonth &&
            expense.category[0] == category.id
        )
    })
    return [lastMonthToDate, MonthToDate]
}
export function From1sttoNowINC(
    incomes: IncomeType[],
    category: CategorySchema
) {
    const lastMonthToDate = incomes.filter((income: IncomeType) => {
        ...
        let lastDayAllowed = new Date()
        return (
            firstDay <= new Date(income.transaction_date) &&
            new Date(income.transaction_date) <= lastDayAllowed
        )
    })
